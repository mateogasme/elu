...
export class EluController {
  ...
  destroy(): void {
    ...
    this.items.forEach((it) => {
      ...
      it.removeEventListener('mousemove', this.onItemHover);
    });
    if (this.searchInput) {
  ...
  private wireEvents(): void {
    ...
    this.items.forEach((it) => {
      ...
      it.addEventListener('mousemove', this.onItemHover);
    });
    if (this.searchInput) {
  ...
  private onItemHover = (e: MouseEvent) => {
    const item = e.currentTarget as HTMLElement;
    if (item.dataset.disabled === 'true') return;
